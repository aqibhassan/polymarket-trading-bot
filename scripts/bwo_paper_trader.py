#!/usr/bin/env python3
"""BWO Paper Trader v2 — Symmetric Entry + Improved BuyBoth on Real Polymarket Prices.
...
v2 FIXES (from failed v1):
  - v1 Hybrid B always bought YES → only profits when BTC UP (50% coin flip)
  - v1 entry prices assumed ~$0.50 but real min-3 prices are $0.60-$0.80
  - v2 Symmetric Entry buys WINNING direction at min 3 (like working 5m strategy)
  - v2 BuyBoth uses higher threshold (0.60 not 0.50) to avoid confident-but-wrong

Strategies:
  Symmetric: At min 3, buy winning direction if model confident + EV-positive
  BuyBoth:   Buy YES+NO pre-window, at min 3 keep winner if confident, else sell both
...
# Strategy A: Symmetric Entry at min 3
SYM_THRESHOLD = 0.65    # Model confidence threshold
SYM_MIN_EDGE = 0.05     # cont_prob must exceed entry_price + this
SYM_MAX_PRICE = 0.75    # Hard cap on entry price (lowered from 0.85 — bad R:R above this)
MIN_BTC_RETURN = 0.0003 # Skip if |btc_return| < 0.03% (noise, not a direction)

# Strategy B: Improved BuyBoth
BB_THRESHOLD = 0.60     # Higher threshold to prevent confident-but-wrong losses
BB_MAX_ENTRY = 0.55     # Pre-window entry cap per side
...
def train_models() -> tuple[Any, Any, list[str]]:
    ...
    # Exclude features unreliable at runtime (DVOL time resolution mismatch)
    _EXCLUDE_RUNTIME = {"dvol_level", "dvol_change_5", "dvol_z"}
    sorted_mi = sorted(mi_results.items(), key=lambda x: x[1], reverse=True)
    feature_names = [
        f for f, mi in sorted_mi
        if mi >= 0.0005 and f not in _EXCLUDE_RUNTIME
    ][:40]
    if len(feature_names) < 5:
        feature_names = [f for f, _ in sorted_mi if f not in _EXCLUDE_RUNTIME][:20]
    log.info(f"  Selected {len(feature_names)} features by MI (excluded DVOL)")
...
def fetch_deribit_dvol() -> float:
    """Fetch latest BTC DVOL from Deribit volatility index data."""
    try:
        ...
        resp = get_client().get(
            ...
            params={
                "currency": "BTC",
                "resolution": 60,
                "start_timestamp": now_ms - 3600000,  # last hour
        ...
        if data:
            return float(data[-1][4])  # close price of latest bar
        return 0.0
    except Exception as e:
        log.warning(f"Deribit DVOL error: {e}")
        return 0.0
...
def compute_runtime_features(
    ...
    # DVOL features excluded from model (time resolution mismatch at runtime)
    dvol_feats: dict[str, float] = {"dvol_level": 0.0, "dvol_change_5": 0.0, "dvol_z": 0.0}
...
def decide_symmetric(
    btc_return: float, cont_prob: float, winner_ask: float,
) -> tuple[str, str]:
    """Symmetric entry decision at minute 3.

    Buy WINNING direction if model confident + EV-positive.
    Returns: (action, side) where action is "BUY" or "SKIP", side is "YES"/"NO".
    """
    if abs(btc_return) < MIN_BTC_RETURN:
        return "SKIP", "FLAT"

    side = "YES" if btc_return > 0 else "NO"

    ev_ok = cont_prob > (winner_ask + SYM_MIN_EDGE)
    conf_ok = cont_prob >= SYM_THRESHOLD
    price_ok = winner_ask <= SYM_MAX_PRICE

    if ev_ok and conf_ok and price_ok:
        return "BUY", side
    return "SKIP", side


def decide_buyboth(
    btc_return: float, cont_prob: float,
) -> str:
    """Improved BuyBoth decision at minute 3.

    Higher threshold (0.60) to prevent confident-but-wrong catastrophic losses.
    Returns: "KEEP_YES", "KEEP_NO", "SELL_BOTH"
    """
    btc_up = btc_return > 0
    btc_down = btc_return < 0
    filter_continue = cont_prob >= BB_THRESHOLD
...
def update_summary(
    sym_bankroll: float, buyboth_bankroll: float,
    sym_trades: list[dict], buyboth_trades: list[dict],
) -> None:
    ...
    summary = {
        ...
        "version": "v2_fixed",
        "symmetric": {
            "bankroll": round(sym_bankroll, 2),
            "stats": _stats(sym_trades),
        },
        "buyboth": {
...
class WindowTracker:
    ...
    bb_entry_done: bool = False  # BuyBoth enters pre-window
    decision_done: bool = False   # Min-3 decisions made
    settlement_done: bool = False
    # Symmetric Entry state (enters at min 3)
    sym_entered: bool = False
    sym_side: str = ""  # "YES" or "NO"
    sym_entry_price: float = 0.0
    sym_shares: float = 0.0
    sym_fee: float = 0.0
    sym_skip_reason: str = ""
    # BuyBoth state (enters pre-window)
...
def main() -> None:
    log.info("=" * 70)
    log.info("BWO PAPER TRADER v2 — Symmetric Entry + Improved BuyBoth")
    log.info(f"  Symmetric: thresh={SYM_THRESHOLD}, edge={SYM_MIN_EDGE}, max=${SYM_MAX_PRICE}")
    log.info(f"  BuyBoth:   thresh={BB_THRESHOLD}, max_entry=${BB_MAX_ENTRY}")
    log.info("=" * 70)

    # Train or load models
    cont_model, pricing_model, feature_names = load_or_train_models()

    # Initialize bankrolls (fresh start for v2)
    sym_bankroll = INITIAL_BANKROLL
    buyboth_bankroll = INITIAL_BANKROLL
    sym_trades: list[dict] = []
    buyboth_trades: list[dict] = []

    # Load existing v2 trades if any
            ...
            for line in f:
                ...
                try:
                    ...
                    if rec.get("version") != "v2":
                        continue  # Skip old v1 trades
                    if "symmetric" in rec and rec["symmetric"].get("entered"):
                        sym_trades.append(rec["symmetric"])
                        sym_bankroll = rec["symmetric"].get("bankroll_after", sym_bankroll)
                    if "buyboth" in rec and rec["buyboth"].get("entered"):
                        buyboth_trades.append(rec["buyboth"])
                        buyboth_bankroll = rec["buyboth"].get("bankroll_after", buyboth_bankroll)
                except json.JSONDecodeError:
                    pass
        if sym_trades or buyboth_trades:
            log.info(f"Resumed v2: {len(sym_trades)} sym, {len(buyboth_trades)} bb trades")
            log.info(f"Bankrolls: Sym=${sym_bankroll:.2f}, BB=${buyboth_bankroll:.2f}")
    ...
    dvol_buffer: list[float] = []  # Rolling DVOL readings (polled each cycle)
    ...
    while True:
        try:
            ...
            if time.time() - last_data_poll >= POLL_INTERVAL - 1:
                ...
                dvol_val = fetch_deribit_dvol()
                if dvol_val > 0:
                    dvol_buffer.append(dvol_val)
                    if len(dvol_buffer) > 100:
                        dvol_buffer = dvol_buffer[-100:]
                last_data_poll = time.time()

            # ============================================================
            # PHASE 1: Pre-window (last 120s of current window)
            # Only BuyBoth enters here. Symmetric waits until min 3.
            # ============================================================
            if elapsed >= 780:
                ...
                # BuyBoth: buy YES+NO pre-window
                if nw.market and not nw.bb_entry_done:
                    ...
                    # BuyBoth entry: both sides must be near $0.50
                    bb_fillable = (yes_ask <= BB_MAX_ENTRY and no_ask <= BB_MAX_ENTRY
                                   and yes_size >= 10)
                    if bb_fillable:
                        bb_half = buyboth_bankroll * POSITION_PCT / 2
                        bb_yes_shares = bb_half / yes_ask
                        bb_no_shares = bb_half / no_ask
                        bb_fee = (polymarket_fee(bb_half, yes_ask)
                                  + polymarket_fee(bb_half, no_ask))
                        nw.bb_entered = True
                        ...
                        nw.bb_fee = bb_fee
                        nw.bb_entry_done = True
                        log.info(
                            f"[BUYBOTH] BUY YES+NO: "
                            f"{bb_yes_shares:.1f}@${yes_ask:.3f} + "
                            f"{bb_no_shares:.1f}@${no_ask:.3f}"
                        )

            # ============================================================
            # PHASE 1b: Early window (first 60s) — continue trying BB entry
            # ============================================================
            if elapsed < 60:
                w = active_windows.get(window_start)
                if w and w.market and not w.bb_entry_done:
                    ...
                    if bb_fillable:
                        bb_half = buyboth_bankroll * POSITION_PCT / 2
                        nw_ref = w
                        nw_ref.bb_entered = True
                        nw_ref.bb_yes_entry = yes_ask
                        nw_ref.bb_no_entry = no_ask
                        nw_ref.bb_yes_shares = bb_half / yes_ask
                        nw_ref.bb_no_shares = bb_half / no_ask
                        nw_ref.bb_fee = (polymarket_fee(bb_half, yes_ask)
                                         + polymarket_fee(bb_half, no_ask))
                        nw_ref.bb_entry_done = True
                        log.info(f"[BUYBOTH EARLY] BUY YES+NO @ ${yes_ask:.3f}+${no_ask:.3f}")

            # ============================================================
            # PHASE 2: Minute 3 decisions (185-300s elapsed)
            # Both Symmetric and BuyBoth make decisions here.
            # ============================================================
            if 185 <= elapsed < 300:
                w = active_windows.get(window_start)
                if w and not w.decision_done and w.market:
                    # Compute features
                    feats = compute_runtime_features(
                        spot_candles, futures_buffer, eth_buffer,
                        deribit_price, dvol_buffer, window_start, feature_names,
                    )
                    ...
                    if feats:
                        ...
                        log.info(
                            ...
                            f"NO bid/ask=${w.min3_no_bid:.3f}/${w.min3_no_ask:.3f}"
                        )

                        # --- SYMMETRIC ENTRY DECISION ---
                        # Buy winning direction at min-3 price if confident
                        winner_ask = w.min3_yes_ask if btc_return > 0 else w.min3_no_ask
                        sym_action, sym_side = decide_symmetric(
                            btc_return, cont_prob, winner_ask
                        )

                        if sym_action == "BUY":
                            sym_pos = sym_bankroll * POSITION_PCT
                            sym_shares = sym_pos / winner_ask
                            sym_fee = polymarket_fee(sym_pos, winner_ask)
                            w.sym_entered = True
                            w.sym_side = sym_side
                            w.sym_entry_price = winner_ask
                            w.sym_shares = sym_shares
                            w.sym_fee = sym_fee
                            expected_ev = cont_prob * (1.0 - winner_ask) - (1.0 - cont_prob) * winner_ask
                            log.info(
                                f"[SYMMETRIC] BUY {sym_side} {sym_shares:.1f} shares "
                                f"@ ${winner_ask:.3f} (EV=${expected_ev:.3f}/unit)"
                            )
                        else:
                            reasons = []
                            if abs(btc_return) < MIN_BTC_RETURN:
                                reasons.append(f"|ret|={abs(btc_return)*100:.4f}%<{MIN_BTC_RETURN*100:.2f}%")
                            if cont_prob < SYM_THRESHOLD:
                                reasons.append(f"prob={cont_prob:.3f}<{SYM_THRESHOLD}")
                            if cont_prob <= winner_ask + SYM_MIN_EDGE:
                                reasons.append(f"EV: need>{winner_ask+SYM_MIN_EDGE:.2f}")
                            if winner_ask > SYM_MAX_PRICE:
                                reasons.append(f"price=${winner_ask:.3f}>${SYM_MAX_PRICE}")
                            w.sym_skip_reason = ", ".join(reasons) if reasons else "flat"
                            log.info(f"[SYMMETRIC] SKIP {sym_side} @ ${winner_ask:.3f}: {w.sym_skip_reason}")

                        # --- BUYBOTH DECISION ---
                        if w.bb_entered:
                            w.bb_action = decide_buyboth(btc_return, cont_prob)
                            ...
                            log.info(f"[BUYBOTH] Decision: {w.bb_action}")

                    else:
                        log.warning("[MIN3] Feature computation failed — skipping")
                        if w.bb_entered:
                            w.bb_action = "SELL_BOTH"

                    w.decision_done = True

            # ============================================================
            # PHASE 3: Settlement at minute 14+ (840s+)
            # ============================================================
            if elapsed >= 840:
                w = active_windows.get(window_start)
                if w and w.decision_done and not w.settlement_done:
                    # Need at least one strategy to have entered
                    if not w.sym_entered and not w.bb_entered:
                        ...
                        if len(window_candles) >= 14:
                            ...
                            # --- Symmetric settlement ---
                            if w.sym_entered:
                                # Settlement depends on which side we bought
                                if w.sym_side == "YES":
                                    settle_price = yes_settles
                                else:
                                    settle_price = no_settles

                                if settle_price > 0:
                                    w.sym_fee += polymarket_fee(
                                        w.sym_shares * settle_price, settle_price
                                    )

                                sym_pnl_gross = w.sym_shares * (settle_price - w.sym_entry_price)
                                sym_pnl_net = sym_pnl_gross - w.sym_fee
                                sym_bankroll += sym_pnl_net

                                sym_rec = {
                                    "entered": True,
                                    "side": w.sym_side,
                                    "entry_price": round(w.sym_entry_price, 4),
                                    "exit_price": round(settle_price, 4),
                                    "shares": round(w.sym_shares, 2),
                                    "pnl_gross": round(sym_pnl_gross, 2),
                                    "fee": round(w.sym_fee, 2),
                                    "pnl_net": round(sym_pnl_net, 2),
                                    "bankroll_after": round(sym_bankroll, 2),
                                }
                                record["symmetric"] = sym_rec
                                sym_trades.append(sym_rec)
                                result = "WIN" if sym_pnl_net > 0 else "LOSS"
                                log.info(
                                    f"[SYMMETRIC] {result} {w.sym_side}@${w.sym_entry_price:.3f} → "
                                    f"${settle_price:.2f}, PnL=${sym_pnl_net:+.2f}, "
                                    f"bankroll=${sym_bankroll:.2f}"
                                )
                            else:
                                record["symmetric"] = {
                                    "entered": False,
                                    "skip_reason": w.sym_skip_reason,
                                }
                            ...
                            if w.bb_entered:
                                ...
                                record["buyboth"] = bb_rec
                                buyboth_trades.append(bb_rec)
                                result = "WIN" if bb_pnl_net > 0 else "LOSS"
                                log.info(
                                    f"[BUYBOTH] {result} {w.bb_action} → "
                                    f"PnL=${bb_pnl_net:+.2f}, bankroll=${buyboth_bankroll:.2f}"
                                )
                            else:
                                record["buyboth"] = {"entered": False}

                            # Log trade
                            log_trade(record)
                            update_summary(sym_bankroll, buyboth_bankroll,
                                           sym_trades, buyboth_trades)
                            w.settlement_done = True
                        else:
                            log.debug(f"[SETTLE] Only {len(window_candles)} candles — waiting")

            # ============================================================
            # Mark windows without market as done
            # ============================================================
            if 200 <= elapsed < 220:
                w = active_windows.get(window_start)
                if w and not w.decision_done and not w.market:
                    log.info(f"[SKIP] Window {window_start} — no market found")
                    w.decision_done = True
                    w.settlement_done = True
