...
import type { BWOConfidenceBucket } from '@/lib/types/bwo';

interface ConfidenceDistributionProps {
  data: BWOConfidenceBucket[];
}
...
export function ConfidenceDistribution({ data }: ConfidenceDistributionProps) {
  if (data.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium text-zinc-400">Win Rate by Confidence</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <Tooltip
              ...
              formatter={(value: number, _: string, props: { payload?: BWOConfidenceBucket }) => {
