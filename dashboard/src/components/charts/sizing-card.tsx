          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="font-mono text-sm text-zinc-200">
                  {(Number(sizing.kelly_fraction) * 100).toFixed(2)}%
                </p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Win Prob</p>
                <p className="font-mono text-sm text-zinc-200">
                  {(Number(sizing.estimated_win_prob) * 100).toFixed(1)}%
                </p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Recommended</p>
                <p className="font-mono text-sm text-zinc-200">${Number(sizing.recommended_size).toFixed(2)}</p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Max Allowed</p>
                <p className="font-mono text-sm text-zinc-200">${Number(sizing.max_allowed).toFixed(2)}</p>
              </div>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[10px] text-zinc-500">
                ...
                <span>{((Number(sizing.recommended_size) / Math.max(Number(sizing.max_allowed), 0.01)) * 100).toFixed(0)}%</span>
              </div>
              <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                <div
                  className="h-full bg-blue-500 rounded-full transition-all"
                  style={{
                    width: `${Math.min(100, (Number(sizing.recommended_size) / Math.max(Number(sizing.max_allowed), 0.01)) * 100)}%`,
                  }}
                />
              </div>
            </div>
