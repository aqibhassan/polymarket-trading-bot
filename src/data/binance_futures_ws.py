...
class BinanceFuturesWSFeed:
    ...
    async def _connection_loop(self) -> None:
        ...
        to REST polling via ``fapi.binance.com`` (CloudFront-backed, works
        from regions where ``fstream.binance.com`` is unreachable).
        """
        while self._running:
            # If too many WS failures and REST fallback enabled, switch mode
            if (
                self._rest_fallback
                ...
                log.warning(
                    "binance_futures_ws.switching_to_rest",
    ...
    async def _rest_poll_loop(self) -> None:
        """Fallback: poll Binance Futures REST API for price data."""
        url = f"{_FAPI_BASE}/fapi/v1/ticker/price?symbol={self._symbol}"
        log.info(
            "binance_futures_rest.starting",
            url=url,
            ...
            while self._running:
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        self._velocity_ticks.append(
                            (time.monotonic(), price),
                        )
                        self._last_msg_time = asyncio.get_event_loop().time()
                        log.debug(
                            "binance_futures_rest.price",
                            price=str(price),
                        )
                    else:
                        log.warning(
                            "binance_futures_rest.error",
                    ...
                    log.warning(
                        "binance_futures_rest.poll_error",
                        exc_info=True,
                    )
